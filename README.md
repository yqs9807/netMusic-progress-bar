...
| `RETRO_BADGE` | 左侧使用绘制圆形黑胶唱片（**播放时旋转，暂停时停止转动**），搭配机械唱臂联动（**播放时唱头斜切压入盘面，暂停时垂直停靠归位**），右侧展示时间数值与状态图标 |
...
```json
{
  ...
  "colors": {
    ...
    "vinylHighlightMain": "#999999",      // 唱片旋转高光色
    "vinylHighlightFade": "#656565",      // 唱片旋转阴影色
...
| **`#141414` 及以上** | **$\ge 20$** | **$\le 5\%$** | **完美兼容（5%~100% 全亮度安全）** |
| `#222222`（默认底槽） | 34 | 3% | 完美兼容 |
| `#2A2A2A`（黑胶唱片） | 42 | 3% | 完美兼容 |

### 一键检测配色方案
在调整 `config.json` 中的自定义配色后，建议运行项目内置的色彩合规检测脚本：

```bash
node check-colors.js
```
